    ...
    return (
        <>
            ...
            <Modal show={deleteModal} size="md" popup onClose={() => setDeleteModal(false)}>
                <Delete product={product} deleteProduct={deleteProduct} setDeleteModal={setDeleteModal}/>
            </Modal>
            <div className='my-4 p-2 flex justify-start items-center'>
                ...
                <p className={`w-48 text-center border rounded-lg py-1 px-2 text-white ${product.is_active ? 'bg-green-600' : 'bg-red-600'}`}>{product.is_active ? 'AKTIF' : 'TIDAK AKTIF'}</p>
                <div className='w-72 flex gap-8 justify-center text-white'>
                    ...
                    <button className='border rounded-lg bg-red-500 py-1 px-2' onClick={() => setDeleteModal(true)}>
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 50 50">
